...
- Morning run outputs only `## Recommended Next Steps` checklist candidates.
- The checklist comes from Hermes-generated `AreaContext` output.
- It updates `Compass.md` to keep exactly two top-level sections: `## Next Steps` and `## Shared Goals`.
- User-owned checkbox items stay in `## Next Steps`.
- Agent candidate recommendations are written in a managed non-checkbox block inside `## Next Steps`.
...
Analyze recommended Next Steps candidates from all active Shared Goals.

1. Prioritize AreaContext `signal` by urgency and contract pressure.
2. Keep balance across four dimensions in LineContext `signal` values when possible.
3. Write a short checklist of 2 to 4 candidate LineContext entries for today's manual selection.
4. Preserve the goal hashtag on every proposed LineContext line.
