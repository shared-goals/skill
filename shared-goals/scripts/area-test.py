...
def check_signal_scope_addresses(area_guidance_section: str) -> tuple[bool, str]:
    ...
    if ambiguous:
        return (
            False,
            "ambiguous `signal` references; use explicit `AreaContext `signal`` or `LineContext `signal``: "
            + " | ".join(ambiguous),
        )
...
def collect_skill_checks(
    area_key: str, area_file: Path, checks: list[CheckResult]
...
def collect_signal_guidance_checks(checks: list[CheckResult], area_sec: str, line_sec: str) -> tuple[bool, str]:
    ...
    add_check(
        checks,
        "line_signal_section_removed",
        not bool(line_sec),
        clean_block(line_sec) if line_sec else "absent",
        "signal_guidance",
    )
...
def collect_infrastructure_checks(
    ...
    add_check(
        checks,
        "boundary_to_area_projection",
        area_ctx_ok,
        "BoundaryAreaContext -> AreaContext projection schema",
        "infrastructure",
    )
    add_check(
        checks,
        "boundary_line_context_schema",
        line_ctx_ok,
        f"LineContext strict schema; lines={len(projected_area['lines'])}",
        "infrastructure",
    )
    add_check(
        checks,
        "boundary_area_signal_empty",
        area_signal_empty,
        f"AreaContext signal={projected_area.get('signal', '')!r}",
        "infrastructure",
    )
    add_check(
        checks,
        "boundary_line_signals_empty",
        line_signals_empty,
        f"all line signals empty; lines={len(projected_area['lines'])}",
        "infrastructure",
    )
    if area_sec:
...
def collect_area_test_checks(area_key: str) -> list[CheckResult]:
    ...
    add_check(
        checks, "area_signal_guidance_section", area_signal_guidance_ok, area_signal_guidance_detail, "signal_guidance"
    )
