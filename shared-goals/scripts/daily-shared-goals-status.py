...
def _commit_source_ref(goal_id: str, completed: list[str], incomplete: list[str]) -> str:
...
def maybe_sync_completed_tasks(path: Path) -> int:
    ...
    for goal in goals:
        ...
        create_commit(
            ...
            happy_moment=happy_moment,
            source_ref=_commit_source_ref(goal_id, goal["completed"], goal["incomplete"]),
        )
        created += 1
    return created
...
def build_platform_lines(payload: dict[str, Any], *, append_goal_tag: bool = False) -> list[dict[str, str]]:
    ...
    for dim in order:
        ...
        for goal in goals:
            ...
            title = " ".join(
                part for part in [goal_title, goal_tag, dim_tags, f"hunger:{hunger}d"] if part
            ).strip()
            if not title:
                continue
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--compass-path",
        default=str((Path(os.environ.get("OBSIDIAN_VAULT_PATH", "")).expanduser() if os.environ.get("OBSIDIAN_VAULT_PATH", "").strip() else Path.home()) / "Compass.md"),
